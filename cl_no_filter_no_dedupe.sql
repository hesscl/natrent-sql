SELECT COUNT(*) AS cl_listing_count, 
       ...
       QUANTILE(CASE WHEN f.clean_beds = 0 THEN f.clean_rent END, .25) AS cl_q25_rent_0b,
       ...
       QUANTILE(CASE WHEN f.clean_beds = 0 THEN f.clean_rent END, .50) AS cl_med_rent_0b,
       ...
       QUANTILE(CASE WHEN f.clean_beds = 0 THEN f.clean_rent END, .75) AS cl_q75_rent_0b,
       ...
       QUANTILE(CASE WHEN f.clean_beds = 0 THEN f.clean_rent END, .95) AS cl_q95_rent_0b,
...
FROM (
      SELECT e.trt_id, e.met_id, e.clean_beds, e.clean_sqft, e.clean_rent, e.match_type, e.post_id, e.lng, e.lat
      FROM (
            ...
            INNER JOIN clean d ON ST_Contains(c.geometry, d.geometry) 
            WHERE d.listing_date BETWEEN '2019-01-01' AND ?end
